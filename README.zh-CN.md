...
| 能力 | 已实践证据 |
|---|---|
| SFT 与蒸馏数据管线 | 批量问答生成（断点续跑、指数退避重试）、带去重与过滤统计的数据清洗、基于实测的接线踩坑记录 |
| 多智能体调度与运营 | 三层架构、结构化调度/汇报提示词模板、派发→跟踪→验收→建议工作流配独立第二模型打分 |
| 面向智能体的提示词设计 | 懒人一键指令、验收标准设计、可复用的双语技术文档 |
| 轻量 Web 交付 | 纯 HTML / CSS / JavaScript、响应式双语页面、Cloudflare Pages 与 GitHub Pages |
